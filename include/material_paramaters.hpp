...
class mat_paramClass
{
    ...
    public:
        mat_paramClass() : hbar{1.05459e-34}, q{1.60219e-19}, eps_0{8.85419e-12}, kb{1.38066e-23},  m_0{9.11e-31},  alpha{0}, v_sound{343.0}, temp{300.0}, vt{temp*kb / ( 1.60219e-19)}, density{2000}, eps{25}, eps_infty{4.5}, effmass{0.15*9.11e-31, 0.15*9.11e-31, 0.15*9.11e-31}{}
        
        void seteffectivemasses (double effmassE, double effmassH, double effmass_ions);
        double get_effmassE ();
        double get_effmassH ();
        double get_effmass_ions ();
        double get_alpha();
        double get_hbar();
        double get_q();
        double get_kb();
	...
	void   set_eps_infty(double epsilon_uv);
        ...
        void mat_paramClass::seteffectivemasses (double effmassE, double effmassH, double effmass_ions)    
        {
            effmass[0] = effmassE;
            effmass[1] = effmassH;
            effmass[2] = effmass_ions;

        }

        double mat_paramClass::get_effmassE () 
        {
            return effmass[0];
        } 

        double mat_paramClass::get_effmassH ()
        {
            return effmass[1];
        } 

        double mat_paramClass::get_effmass_ions ()
        {
            return effmass[2];
        } 
...
{   
    ...
    public: 
        scat_paramClass(int ac, int op) : def_pot{-2.13 * 1.6e-19}, polarw0{2.25e12*2*M_PI}, acousticscattering(ac),opticalscattering(op){}
